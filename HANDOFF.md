...
- GitHub 使用外部連結開啟 `https://github.com/superstar-boop`。
- LinkedIn 與 Send Feedback 保留為 disabled 設定列。
- Splash 使用既有正式 App Logo，顯示 Built by／Mark Hu；沒有增加計時器、外部請求或人為啟動延遲。
- Runtime 作者資訊集中在 `brandAttribution`，未來只需修改單一設定物件即可同步 About 與 Splash。
...
- About 開啟、返回設定、GitHub 外開及 disabled 項目狀態通過。
- Splash 作者字級為 40px，Logo 為 112px，比例約 35.7%；頁面載入完成後立即隱藏，沒有額外等待時間。
...
- 指定的 GitHub 網址可正常外開，但 GitHub 目前顯示 Page not found；需由 Product Owner 確認 `superstar-boop` 是否為正確或尚未建立的帳號。
